...
void effSDC_hyps( void )
{
  ...
  // draw Multi with plane efficiency SDC0
  {
    const Int_t n_layer = 4;
    TCanvas *c = (TCanvas*)gROOT->FindObject("c1");
    c->Clear();
    c->Divide(2,2);
    Int_t base_id = HistMaker::getUniqueID(kSDC0, 0, kMulti, 0);
  ...
  // draw Multi with plane efficiency SDC1
  {
    ...
    c->Clear();
    c->Divide(3,2);
    Int_t base_id = HistMaker::getUniqueID(kSDC1, 0, kMulti, 0);
    //    Int_t base_id_ctot = HistMaker::getUniqueID(kSDC1, 0, kMulti, 1+kTOTcutOffset);
    for( Int_t i=0; i<n_layer; ++i ){
      c->cd(i+1);
  ...
  {
    const Int_t n_layer = 5;
    TCanvas *c = (TCanvas*)gROOT->FindObject("c3");
    c->Clear();
    c->Divide(3,2);
    Int_t base_id = HistMaker::getUniqueID(kSDC2, 0, kMulti, 0);
    for( Int_t i=0; i<n_layer; ++i ){
      c->cd(i+1);
      TH1 *h_wt = GHist::get(base_id +i +n_layer);
      h_wt->Draw();
    ...
    // draw Multi with plane efficiency SDC3
  {
    ...
    c->Clear();
    c->Divide(3,2);
    Int_t base_id = HistMaker::getUniqueID(kSDC3, 0, kMulti, 0);
    for( Int_t i=0; i<n_layer; ++i ){
      c->cd(i+1);
      TH1 *h_wt = GHist::get(base_id +i +n_layer);
      h_wt->Draw();
