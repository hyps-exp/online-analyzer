...
void dispSDC0_hyps( void )
{
  ...
  // draw TDC
  {
    ...
    c->Clear();
    c->Divide(2,2);
    int base_id = HistMaker::getUniqueID(kSDC0, 0, kTDC, 0);
    int base_id_ctot = HistMaker::getUniqueID(kSDC0, 0, kTDC, kTOTcutOffset);
  ...
  // draw TDC1st
  {
    ...
    c->Clear();
    c->Divide(2,2);
    int base_id = HistMaker::getUniqueID(kSDC0, 0, kTDC2D, 0);
    int base_id_ctot = HistMaker::getUniqueID(kSDC0, 0, kTDC2D, kTOTcutOffset);
    for( int i=0; i<n_layer; ++i ){
      c->cd(i+1);
      TH1 *h = (TH1*)GHist::get(base_id + i);;
      if( !h ) continue;
  ...
  // draw TOT
  {
    ...
    c->Clear();
    c->Divide(2,2);
    int base_id = HistMaker::getUniqueID(kSDC0, 0, kADC, 0);
    int base_id_ctot = HistMaker::getUniqueID(kSDC0, 0, kADC, kTOTcutOffset);
    int base_id_tot1st = HistMaker::getUniqueID(kSDC0, 0, kTDC2D,  10+kTOTcutOffset);
    for( int i=0; i<n_layer; ++i ){
      c->cd(i+1);
      TH1 *h = (TH1*)GHist::get(base_id + i);;
      if( !h ) continue;
  ...
  // draw HitPat
  {
    ...
    c->Clear();
    c->Divide(2,2);
    int base_id = HistMaker::getUniqueID(kSDC0, 0, kHitPat, 0);
    int base_id_ctot = HistMaker::getUniqueID(kSDC0, 0, kHitPat, kTOTcutOffset);
    for( int i=0; i<n_layer; ++i ){
      c->cd(i+1);
      TH1 *h = (TH1*)GHist::get(base_id + i);
      if( !h ) continue;
  ...
  // draw Multi
  {
    ...
    c->Clear();
    c->Divide(2,2);
    int base_id = HistMaker::getUniqueID(kSDC0, 0, kMulti, 0);
    for( int i=0; i<n_layer; ++i ){
      c->cd(i+1);
...
//      h_wot->SetMaximum(h_wt->GetMaximum()*1.1);
  ...
  // draw TDC2D
  {
    ...
    c->Clear();
    c->Divide(2,2);
    int base_id = HistMaker::getUniqueID(kSDC0, 0, kTDC2D, 20+kTOTcutOffset);
  ...
  // draw TDC2D wTOTCUT
  {
    ...
    int base_id_ctot = HistMaker::getUniqueID(kSDC0, 0, kTDC2D, 30+kTOTcutOffset);
     for( int i=0; i<n_layer; ++i ){
      c->cd(i+1);
